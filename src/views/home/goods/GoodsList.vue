...
<script>
import { getGoodsList_req,deleteGood_req } from '@/network/goods-list'
export default {
  name: 'GoodsList',
  data() {
    return {
  ...
  methods: {
    // 根据分页获取对应的商品列表
    async getGoodsList() {
      const { data: res } = await getGoodsList_req(this.queryInfo)
    ...
    async removeById(id) {
      ...
      const { data: res } = await deleteGood_req(id)
