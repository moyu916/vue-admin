...
export default {
  ...
  methods: {
    async getUserList() {
      const { data: res } = await getUserList_req(this.params)
      // console.log(res)
    ...
    async userStateChanged(userinfo) {
      const { data: res } = await modifyUserStatus_req(userinfo)
      if (res.meta.status !== 200) {
    ...
    addUser() {
      this.$refs.addFormRef.validate(async valid => {
        if (!valid) return
        // 可以发起添加用户的网络请求
        const { data: res } = await addUser_req(this.addForm)
    ...
    async showEditDialog(id) {
      // console.log(id)
      const { data: res } = await getUserInfo_req(id)
    ...
    editUserInfo() {
      this.$refs.editFormRef.validate(async valid => {
        if (!valid) return
        // 发起修改用户信息的数据请求
        const { data: res } = await modifyUserInfo_req(this.editForm)
    ...
    async saveRoleInfo() {
      ...
      const { data: res } = await saveRoleInfo_req(this.userInfo.id, { rid: this.selectedRoleId })
